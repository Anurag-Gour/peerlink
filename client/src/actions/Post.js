...
export const likePost = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`/api/v1/post/like-dislike-post/${id}`);
...
export const deleteCommentOnPost = (id, commentId) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(`/api/v1/post/delete-comment/${id}`, {
      data: { commentId },
    });
...
export const updatePost = (id, caption) => async (dispatch) => {
  try {
    ...
    dispatch({
      ...
      payload: error.respodeletePostage,
    });
  }
};
